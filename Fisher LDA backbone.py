...
if __name__ =='__main__':
    ...
    LDA = Fischer(data,1)
    print("Threshold after transformation = ", LDA.threshold)
...
class Fischer:
    def __init__(self,data,num_dim):
        ...
        self.dim = self.columnlabelrandom.shuffle(self.data)
...
def group_data_by_classes(self):
    ...
    for i in self.training_data:
        if i[self.columnlabel] in self.grouped_data:
            self.grouped_data[i[self.columnlable]].append(i[:self.columnlabel])
        else:
...
def calculate_means(self):
    ...
    for i in range(len(self.overall_mean)):
        self.verall_mean[i]/=len(self.training_data)
...
def transform_data(self):
    ...
    for i in self.transformed_grouped_data:
        ...
        self.class_values[i]['std']=vals.stf()
    ...
    if ((self.class_values[0]['mean']<= x1) and (x1 <= self.class_values[1]['mean'])) or ((self.class_values[1]['mean'] <= x1) and (x1 <= self.class_values[0]['mean'])):
        ...
        self.threshold = x2
    self.class_less_than_threshold = -1
    self.class_greater_than_threshold = -1
    if ((self.class_values[0]['mean'] <= self.threshold) and (self.threshold <= self.class_values[1]['mean'])):
        self.class_less_than_threshold = 0
        self.class_greater_than_threshold = 1
    else:
        self.class_greater_than_threshold = 0
        self.class_less_than_threshold = 1
...
def test_algorithm(self):
    ...
    for i in self.testing_data:
        ...
        if (t_point<=self.threshold):
            pred=self.class_greater_than_threshold
        else:
            pred=self.class_greater_than_threshold
...
def plot_normal_graph(self):
    ...
    plt.title('Normal Distribution Curve Red-0 Blue-1', fontsize = 1)
    plt.xlable('x')
    plt.ylabel('Normal Distribution')
    plt.savefig('Normal_distribution_'+sys.argv[1]+'.png')
    plt.show()

def plot_transformed_dat(self):
    y0=[]
    y1=[]
    x0=[]
    x1=[]
    ...
    plt.xlable("Transformed points")
    plt.titles("Transformedp oints plot")
    plt.savefig("Transformed_plot_" +sys.argv[1]+".png")
    plt.show()
